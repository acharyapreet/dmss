...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/dashboard/stats`, {
  ...
  const handleUpdateProfile = async () => {
    ...
    console.log('Starting profile update with data:', editForm)
    console.log('Using token:', token.substring(0, 20) + '...')
    
    setLoading(true)
    try {
      const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/auth/profile`
  ...
  const handleChangePassword = async () => {
    ...
    setLoading(true)
    try {
      const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/auth/change-password`
      console.log('Making password change request to:', apiUrl)
  ...
  const handleUpdateNotifications = async () => {
    if (!token) {
      console.error('No token available for notification settings update')
      alert('Authentication token not found. Please log in again.')
      return
    }
    
    console.log('Starting notification settings update with data:', notificationSettings)
    console.log('Using token:', token.substring(0, 20) + '...')
    
    setLoading(true)
    try {
      const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/auth/notifications`
      console.log('Making notification settings request to:', apiUrl)
      ...
      console.log('Notification settings response ok:', response.ok)
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        console.error('Notification settings update failed:', data)
  ...
  const handleUpdatePreferences = async () => {
    if (!token) {
      console.error('No token available for preferences update')
      alert('Authentication token not found. Please log in again.')
      return
    }
    
    console.log('Starting preferences update with data:', preferences)
    console.log('Using token:', token.substring(0, 20) + '...')
    
    setLoading(true)
    try {
      const apiUrl = `${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/auth/preferences`
      console.log('Making preferences request to:', apiUrl)
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="flex items-center gap-3 p-3 rounded-lg bg-secondary/30">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-foreground">January 30, 2026 at 9:00 AM</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 rounded-lg bg-secondary/30">
