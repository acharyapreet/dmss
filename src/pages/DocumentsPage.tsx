...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        // Note: Don't exclude archived by default since there might not be any archived documents
        
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/documents?${queryParams}`, {
          headers: {
  ...
  const handleEditDocument = async (doc: any) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5004/api'}/documents/${doc._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
  ...
  const filteredDocuments = documents.filter((doc: any) => {
    ...
    // If showing archived, only show archived documents
    // If not showing archived, exclude archived documents
    const matchesArchiveFilter = showArchived ? doc.status === 'archived' : doc.status !== 'archived'
    
    return matchesSearch && matchesType && matchesStatus && matchesArchiveFilter
  })
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <Card>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{documents.filter(d => d.status === "draft").length}</p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{documents.filter(d => d.status === "pending").length}</p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{documents.filter(d => d.status === "approved").length}</p>
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-foreground">{documents.filter(d => d.status === "rejected").length}</p>
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                      <div className="flex items-start gap-4 flex-1">
                        <div className={`h-12 w-12 rounded-lg flex items-center justify-center ${
                          ...
                          doc.status === "pending" ? "bg-warning/10" : "bg-primary/10"
                        }`}>
                          <FileText className={`w-6 h-6 ${
                            doc.status === "approved" ? "text-success" :
                            doc.status === "rejected" ? "text-destructive" :
                            doc.status === "pending" ? "text-warning-foreground" : "text-primary"
                          }`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            {doc.type} • Created by {doc.owner?.fullName || doc.owner?.firstName + ' ' + doc.owner?.lastName || 'Unknown'}
                          </p>
          ...
          <DialogContent className="sm:max-w-[525px]">
            ...
            {selectedDocument && (
              <div className="grid gap-4 py-4">
                <div className="grid gap-2">
                  <Label>Title</Label>
                  <p className="text-sm text-foreground">{selectedDocument.title}</p>
                </div>
                <div className="grid gap-2">
                  <Label>Type</Label>
                  <p className="text-sm text-foreground">{selectedDocument.type}</p>
                </div>
                <div className="grid gap-2">
                  <Label>Status</Label>
                ...
                <div className="grid gap-2">
                  ...
                  <p className="text-sm text-foreground">{selectedDocument.description || 'No description provided'}</p>
                </div>
                <div className="grid gap-2">
                  <Label>Created By</Label>
                  <p className="text-sm text-foreground">{selectedDocument.owner?.fullName || selectedDocument.owner?.firstName + ' ' + selectedDocument.owner?.lastName || 'Unknown'}</p>
                </div>
                <div className="grid gap-2">
                  <Label>Created Date</Label>
                  <p className="text-sm text-foreground">{new Date(selectedDocument.createdAt).toLocaleString()}</p>
                </div>
              </div>
            )}
            <DialogFooter>
