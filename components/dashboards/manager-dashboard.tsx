...
export function ManagerDashboard() {
    ...
    const fetchManagerData = async () => {
      ...
      try {
        ...
        const [statsResponse, docsResponse, workflowsResponse, caseFilesResponse] = await Promise.all([
          ...
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5004/api'}/documents?status=pending&limit=5`, {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5004/api'}/workflows?status=in-progress&limit=5`, {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5004/api'}/case-files?status=in-progress&limit=5`, {
            headers: { 'Authorization': `Bearer ${token}` }
          })
        ])
  ...
  const handleDocumentAction = async (document: any, action: 'approve' | 'reject') => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5004/api'}/documents/${document._id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleViewHistory = async () => {
    ...
    try {
      ...
      const historyResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5004/api'}/audit`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
